...
export const getAll = asyncHandler(async (req: Request, res: Response) => {
  ...
  const numOfBeds = req.query.numOfBeds
...
// @Method POST
export const createRoomReview = asyncHandler(
  async (req: IUserRequest, res: Response) => {
    const room = await Room.findById(req.params.id);

    if (room) {
      const alreadyReviewd = room.reviews?.find(
      ...
      room.ratings =
        room.reviews?.reduce((acc: any, item: any) => item?.rating + acc, 0) /
        Number(room.reviews?.length);
